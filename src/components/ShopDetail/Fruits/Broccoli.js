...
const Broccoli = () => {
  ...
  const [productt, setProductt] = useState(productts);

  const inputHandler = (e) => {
    const { name, value } = e.target;

    setProductt({ ...productt, [name]: value });
  };

  const submitForm = async (e) => {
    e.preventDefault();

    if (!productt.name || !productt.email || !productt.review) {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/createSDreview",
        productt
      );
      toast.success(response.data.msg);

      // Reset the product state to clear the form fields
      setProductt({
        name: "",
        email: "",
        review: "",
  ...
  return (
      <div className="row g-4">
        ...
        <div className="col-lg-6">
          ...
          <Link className="btn border border-secondary rounded-pill px-4 py-2 mb-4 text-primary">
